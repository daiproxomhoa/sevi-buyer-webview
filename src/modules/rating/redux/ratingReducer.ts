...
export const setPendingRateData = createCustomAction('rating/setPendingRateData', (data?: some) => ({
  data,
}));

export const setLoading = createCustomAction('rating/setLoading', (data: boolean) => ({
  data,
}));
export const setDisableLoadMore = createCustomAction('rating/setDisableLoadMore', (data: boolean) => ({
  data,
}));
  ...
  return async (dispatch, getState) => {
    ...
    if (json.status === SUCCESS_CODE) {
      dispatch(
        setPendingRateData({
          ...json.body,
          requests: [...get(pendingRateData, 'requests', []), ...get(json.body, 'requests', [])],
        }),
      );
      if (json.body?.requests?.length < PAGE_OFFSET) {
        dispatch(setDisableLoadMore(true));
      }
    }
