...
export const useMessages = (options: FetchMessagesParameters): HookReturnValue => {
  ...
  const [page, setPage] = useState<number>(undefined as any);
  ...
  return [messages, command, error as any];
};

const mergeMessageArray = (oldMessages, newMessages) => {
  if (Array.isArray(oldMessages)) {
    return [...oldMessages, ...newMessages].sort((a, b) => (a.timetoken as number) - (b.timetoken as number));
  }
};
