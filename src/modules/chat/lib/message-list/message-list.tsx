...
export const MessageList: FC<MessageListProps> = (props: MessageListProps) => {
  ...
  const spinnerObserver = useRef(new IntersectionObserver((e) => e[0].isIntersecting === true && fetchMoreHistory()));
  const bottomObserver = useRef(new IntersectionObserver((e) => handleBottomScroll(e[0].isIntersecting)));
  ...
  */
  ...
  const handleHistoryFetch = useAtomCallback(
    useCallback((get, set, response: FetchMessagesResponse) => {
      ...
      setEmojiPickerShown(false);
      setPrevMessages(allMessages as any);
  ...
  */
  ...
  useEffect(() => {
    ...
    if (!scrolledBottom && messagesFromListener) setUnreadMessages(unreadMessages + messagesFromListener);

    setupBottomObserver();
    setPrevMessages(messages as any);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [messages]);
  ...
  const renderAttachment = (attachment: ImageAttachment | LinkAttachment, key: number) => {
    return (
      <div key={key} className="pn-msg__attachments">
        {attachment.type === 'image' && <img className="pn-msg__image" src={attachment.image?.source} alt="" />}

        {attachment.type === 'link' && (
          <a className="pn-msg__link" href={attachment.provider?.url} target="_blank" rel="noreferrer noopener">
            <img src={attachment.image?.source} alt="" />
            <div>
              <p className="pn-msg__link-name">
                <img src={attachment.icon?.source} alt="" />
    ...
    <div className={`pn-msg-list pn-msg-list--${theme}`}>
      ...
      <div className="pn-msg-list-scroller" onScroll={props.onScroll} ref={listRef}>
        {!!props.fetchMessages && !paginationEnd && (
          <div
            ref={spinnerRef}
            className="pn-msg-list__spinner"
            style={{ display: 'flex', justifyContent: 'center', height: 44, alignItems: 'center', overflow: 'hidden' }}
          >
        ...
        {(!props.fetchMessages || (!fetchingMessages && !messages.length)) && renderWelcomeMessages()}
        ...
        {props.reactionsPicker && (
          <div
            className={`pn-msg-list__emoji-picker ${!emojiPickerShown && 'pn-msg-list__emoji-picker-hidden'}`}
            ref={pickerRef}
          >
            {picker}
          </div>
