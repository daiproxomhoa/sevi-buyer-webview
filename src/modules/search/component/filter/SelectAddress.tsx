import { Button, fade, ListItem, Popover, Theme, Typography, withStyles } from '@material-ui/core';
...
const SelectAddress = (props: Props) => {
  const { filter, profile, setFilter, onAddNewAddress } = props;
  ...
  return (
    <>
      ...
      >
        ...
        >
          <RadioButtonCheckedIcon color="primary" style={{ fontSize: '20px' }} />
        </div>
      ...
      >
        {profile?.addresses?.map((one: some, index: number) => (
          <ListItem
            ...
            button
            selected={filter.address.address.formattedAddress === one?.address?.formattedAddress}
            style={{
              display: 'flex',
              flexDirection: 'column',
              alignItems: 'flex-start',
        ...
        >
          <Typography variant="body2" color="primary">
            <FormattedMessage id="search.filter.addNewAddress" />
          </Typography>
        </ListItem>
      </CustomPopover>
    </>
