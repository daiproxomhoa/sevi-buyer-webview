...
const VerifyOtpPage = (props: Props) => {
  ...
  const onResend = useCallback(async () => {
    dispatch(setLoadingBackDrop(true));
    const json = await dispatch(fetchThunk(`${API_PATHS.otp}?id=${signUpData.id}`, 'post'));
    dispatch(setLoadingBackDrop(false));
  ...
  return (
    <PageWrapper>
      ...
      <CountDown key={counting} num={counting} onResend={onResend} onCompleted={() => setCounting(0)} />
